...
export default class TagGenerator {
  ...
  parseTagLine(line) {
    ...
    if (sections.length > 3) {
      ...
      // COMBAK : This doesn't seem like the right place for this logic.
  ...
  generate() {
    ...
    const args = [`--options=${defaultCtagsFile}`, '--fields=KsS'];
          ...
          return (() => {
            ...
            for (const line of Array.from(lines.split('\n'))) {
              ...
              if (tag = this.parseTagLine(line)) {
                // TODO KINDS : move this somewhere better
                if (['function', 'class', 'method'].indexOf(tag.type) != -1) {
